...
The page refreshes visible client status every two seconds. The node samples WireGuard runtime state every five seconds, so a new handshake or transfer normally appears within a few seconds. Use the enable switch to bring a stored client up or down, **Edit** to change it, and **Delete** to remove its configuration and tunnel interface.
...
| **Disconnected** | The client is enabled but its interface is unavailable, its runtime state cannot be read, or its latest handshake is stale. |
| **Disabled** | The client is stored but its tunnel interface is not active. |

Transfer values are cumulative for the managed client across interface restarts. By comparison, `wg show` reports counters for the current interface lifetime.
