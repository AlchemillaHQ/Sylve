...
Network objects are named, reusable values for addresses, networks, ports, identities, and remote address sets. Create an object once, then select or reference it wherever Sylve accepts that type. Some fields also take typed values directly, such as firewall addresses, switch addresses, routes, and the IP address on a DHCP static lease, while a lease's MAC or DUID identifier remains object-backed.
