...
An existing DHCP or static `ifconfig_<port>` entry may remain in `/etc/rc.conf`. FreeBSD can use it to configure the bare NIC during early boot; after networking is up, Sylve stops any DHCP client on the selected bridge member, clears its live addresses, and applies the Standard Switch configuration to the bridge. Sylve does not rewrite the `rc.conf` entry. Do not also configure the Sylve-generated bridge in `rc.conf`.
...
If you are connected to Sylve through that interface, expect the connection to drop while the change is applied. Configure the intended address, gateway, DHCP, or SLAAC mode on the Standard Switch in the same operation, and use out-of-band access or a maintenance window if losing network access would be disruptive. After creation, manage the node's live IP configuration on the bridge; an `rc.conf` entry for the physical port serves only as the early-boot or fallback configuration described above.
:::
