---
title: Data Center
description: Manage Data Center notes, shared guest IDs, and cluster recovery from the CLI and interactive console.
---
...
List the authoritative Raft membership, including addresses, suffrage (voting role), Sylve versions and build commits, and the current leader:

```bash
doas sylve datacenter cluster members
```

The **VERSION** column includes the reported build commit when available, for example `0.3.0 - 21a5f0f`. If the commit is missing or reported as `unknown`, only the version is shown. Comparing commits helps identify different builds that report the same version, particularly when using `tip` builds.
...
After a cooperative leave that keeps guests on the departing node, the surviving cluster releases that node's claims once membership removal completes. Those numbers become available to other members. The departed node still has its local guests; if it later rejoins, every current guest ID must be free in the cluster. A reused number blocks the join, including when one side is a VM RID and the other is a jail CTID.
...
The normal reclaim requires a clean inventory from the cluster and is rejected if the ID is registered on any reporting node or involved in an active operation. It also requires working cluster consensus. A cooperative leave already releases its guest IDs; manual reclaim remains relevant for claims left by other removal paths, such as force removal. Current-voter inventory cannot inspect a departed machine, so verify its runtime and storage separately before manually reclaiming one of its IDs.
