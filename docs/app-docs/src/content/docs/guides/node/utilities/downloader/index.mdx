...
Wait for the download and any processing to reach **Done**, then shut the VM off and open **Storage → Add storage**:
...
For a writable disk, leave the optional target size empty to use the image's virtual capacity, or choose a larger capacity. A larger disk does not automatically expand partitions or filesystems inside the guest. See [VM Storage](/guides/node/virtual-machines/storage/) for the complete workflow.
