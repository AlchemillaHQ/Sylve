...
import advanced from '../../../one-shot-guides/simple-jail/simple-jail-create-advanced.png';
...
Sylve clears these during the upgrade so they can be rebuilt with corrected metadata. Removed entries show as **Not created** and can be started again here; building one needs **pkg 2.4.0 or newer** on the node. Existing jails are not changed, but a jail copied from an older bootstrap keeps its metadata — see [Checking a jail from an older bootstrap](/guides/node/jails/updates/#checking-a-jail-from-an-older-bootstrap).
:::
