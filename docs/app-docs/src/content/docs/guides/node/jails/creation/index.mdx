...
{/* Screenshot update needed: jail-create-advanced.png. Capture the current Advanced tab with Type set to FreeBSD, the default allowed options visible, and all lifecycle hooks disabled. The Start and Stop rows must not show /bin/sh /etc/rc or /bin/sh /etc/rc.shutdown as enabled custom hooks. */}
