...
- Fstab content, its captured jail-root mountpoint, and resolver content
...
When a jail is created from the template, Sylve automatically rebases fstab destination
mountpoints beneath the captured jail root onto the new jail's actual root mountpoint. It
does not replace CTID text globally: source paths, mount options, comments, unrelated
numbers, and destinations outside the captured jail root remain unchanged. This also
supports creating the jail on a pool with a different mountpoint.

For legacy templates that do not contain the captured root, Sylve first tries to recover it
from the source jail. If the source no longer exists, jail creation still succeeds, but Sylve
preserves the original fstab entries as comments with a warning. Review their destination
paths and uncomment them when they are ready to use.
...
5. External fstab source paths and any storage that was not part of the captured base tree.
