...
| Read Users | [PAM users](/guides/node/settings/authentication/users/pam/) with a Samba credential, granted read access. |
| Write Users | PAM users with a Samba credential, granted write access. Write access takes precedence, so a user in both lists is kept only as a writer. |
...
:::note
Local Sylve users cannot be share principals: Sylve-only accounts have no host account, so Samba cannot authenticate them. Create or import a PAM user and enable their Samba credential (**Samba User** when creating, or **Create or update Samba user** when editing) before adding them to a share.
:::
