...
{/* Screenshot update needed: snapshots-create.png. Capture the current Create Snapshot dialog with Interval Type set to a recurring schedule and the Retention selector visible. Use either Simple retention with its current defaults or GFS retention with all current default values visible. */}
...
{/* Screenshot update needed: snapshots-periodic-jobs.png. Capture the current View Snapshot Jobs dialog for a selected pool with at least two jobs. The Runs column should show stacked Last and Next values. Ideally include one completed job and one new job whose Last value is Never. Include the ID, Dataset, Prefix, Interval, retention, and delete controls. */}

<img src={snapshotsPeriodicJobs.src} alt="View Snapshot Jobs dialog showing periodic jobs with stacked Last and Next run times" />
...
{/* Screenshot update needed: snapshots-periodic-jobs-retention.png. Capture the current Retention Policies dialog opened from a job. Show Interval Type, its schedule control, the Retention selector, and the selected retention values. Keep the View Snapshot Jobs dialog visible behind it with the new Last and Next run display. */}
