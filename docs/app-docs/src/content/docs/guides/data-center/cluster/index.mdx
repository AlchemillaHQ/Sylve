...
| **Sylve Version** | Version and build commit reported by the member. The commit is shown when available. Membership changes require every participating node to report the same version. |
