...
import standardSwitchCreate from "../simple-jail/simple-jail-standard-switch-create.png";
import standardSwitchExternalConfigurationWarning from "../simple-jail/simple-jail-standard-switch-address-warning.png";
...
import vmEjectInstaller from "./simple-vm-eject-installer.png";
...
3. Select `zroot` under **Storage Pool**, or the pool you want to use for the VM disk.
4. Enter `48 GiB` for **Disk Size**.
5. Select **NVMe** under **Emulation Type**.
...
The creation wizard configures both attachments together. You do not need to use **Add storage** separately for this installation. After creation, the VM's **Storage** page lists the writable disk as **ZVOL** with **RW** access and the read-only ISO as **Media** with **RO** access.

The screenshot below uses Debian 13.5 media. Select the installer downloaded earlier for this guide.

<img
  src={vmCreateStorage.src}
  alt="Create Virtual Machine Storage tab with ZFS Volume selected, Debian 13.5 installation media, storage pool zroot, a 48 GiB disk, and NVMe emulation"
/>
...
<img
  ...
  alt="Create VM wizard Network tab with WAN selected, VirtIO emulation, and no manually selected MAC address"
/>
...
Once the VM is powered off, open its **Storage** page:

1. Select the Debian installer ISO row. Confirm that **Backing** is **Media** and **Access** is **RO**, so you do not select the writable system disk.
2. Select **Eject** and wait for the action to complete.
3. Confirm that the media row now shows **Disconnected** and the system disk remains **Connected**.

Ejecting leaves the attachment in the table and keeps the source ISO in Downloader. The VM will no longer boot from that media. To use it again, shut the VM off, select the same row, and choose **Insert**. You do not need to delete the attachment or add it again.

Storage attachments cannot be changed while the VM is running. If Debian already booted successfully from the virtual disk, shut it down cleanly before ejecting the ISO.

The example below uses a VM named `Debian-13` with Debian 13.5 media and a 32 GiB system disk. Select the installer media in your own VM, not its system disk.

<img
  src={vmEjectInstaller.src}
  alt="Storage page for the shut-off Debian-13 VM with connected read-only Debian installer media selected and the Eject action visible, alongside a connected writable 32 GiB ZVOL disk"
/>
...
The VM is now installed and connected through the `WAN` Standard Switch. For more detail about graphical and serial access, see [VM Console](/guides/node/virtual-machines/console/). For disk and installation-media management, see [VM Storage](/guides/node/virtual-machines/storage/).
