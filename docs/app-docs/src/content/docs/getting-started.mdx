...
That said, if your use case requires maximum stability and you do not need the latest features or bug fixes, **15.0-RELEASE** should work perfectly fine.
:::
...
:::caution
In the ports tree version v0.2.3 by default the enablePAM has been set to `true` this might be a footgun for you when you have users configured on the system that does NOT need to access Sylve's Web UI, as Sylve doesn't support authorization yet just authentication via PAM. Setting this to `false` disables PAM based login completely and you can use regular Local authentication that is maintained in Sylve's sqlite database.

You can also set the `password` for the admin user to be empty after initial setup is done, Sylve will ignore this field when it's set to empty.
:::
