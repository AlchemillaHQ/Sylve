...
type ServiceRegistry struct {
	...
	ReplicationService replicationServiceInterfaces.ReplicationServiceInterface
}
...
func NewService[T any](db *gorm.DB, dependencies ...interface{}) interface{} {
	...
	case *replication.Service:
		authService := dependencies[0].(serviceInterfaces.AuthServiceInterface)
		gzfs := dependencies[1].(*gzfs.Client)
		clusterService := dependencies[2].(*cluster.Service)
		return replication.NewService(db, authService, gzfs, clusterService)
	default:
		return nil
	}
}
...
func NewServiceRegistry(db *gorm.DB) *ServiceRegistry {
	...
	replicationService := NewService[replication.Service](db, authService, gzfs, clusterService)
	...
	return &ServiceRegistry{
		...
		ReplicationService: replicationService.(replicationServiceInterfaces.ReplicationServiceInterface),
	}
}
