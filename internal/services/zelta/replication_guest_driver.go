...
const replicationFenceReasonPolicyOwnerMismatch = "policy_owner_mismatch"
...
type replicationGuestDriver interface {
	...
	selfFence(ctx context.Context, policyID uint, guestID uint, localNodeID, expectedOwner string)
}
...
func (d jailReplicationGuestDriver) selfFence(
	...
	if retireErr != nil {
		logger.L.Warn().
			...
			Str("reason", replicationFenceReasonPolicyOwnerMismatch).
