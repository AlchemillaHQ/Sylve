...
func TestNetworkHandlerMapsServiceErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "manual switch conflict", err: errors.New("filtered_switch_qinq_unsupported: LAN"), wantStatus: http.StatusConflict},
		...
		{name: "manual switch unavailable", err: errors.New("failed_to_inspect_manual_switch_vlan_state: LAN: ioctl failed"), wantStatus: http.StatusServiceUnavailable},
