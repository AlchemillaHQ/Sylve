...
func TestDHCPLeaseHandlersReturnStableValidationAndResourceErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing ip source", method: http.MethodPost, path: "/network/dhcp/lease", body: []byte(`{"hostname":"client-b","macId":` + strconvUint(fixture.macObject.ID) + `,"dhcpRangeId":` + strconvUint(fixture.dhcpRange.ID) + `}`), wantStatus: http.StatusBadRequest, wantCode: "dhcp_ip_required"},
		{name: "conflicting ip sources", method: http.MethodPost, path: "/network/dhcp/lease", body: []byte(`{"hostname":"client-b","ipId":` + strconvUint(fixture.ipObject.ID) + `,"ipRaw":"192.0.2.90","macId":` + strconvUint(fixture.macObject.ID) + `,"dhcpRangeId":` + strconvUint(fixture.dhcpRange.ID) + `}`), wantStatus: http.StatusBadRequest, wantCode: "dhcp_ip_source_conflict"},
		{name: "literal family mismatch", method: http.MethodPost, path: "/network/dhcp/lease", body: []byte(`{"hostname":"client-b","ipRaw":"2001:db8::5","macId":` + strconvUint(fixture.macObject.ID) + `,"dhcpRangeId":` + strconvUint(fixture.dhcpRange.ID) + `}`), wantStatus: http.StatusBadRequest, wantCode: "dhcp_ip_family_mismatch"},
