...
func TestRegisteredStaticRouteRoutesMatchSourceAnnotations(t *testing.T) {
	...
	handlerSource, err := os.ReadFile(filepath.Join(handlerDir, "route.go"))
	if err != nil {
		t.Fatal(err)
	}

	registered := map[string]struct{}{}
	routePattern := regexp.MustCompile(`(?m)^\s*routes\.(GET|POST|PUT|PATCH|DELETE)\("([^"]*)"`)
	for _, match := range routePattern.FindAllStringSubmatch(string(routesSource), -1) {
		path := regexp.MustCompile(`:([A-Za-z0-9_]+)`).ReplaceAllString("/network/route"+match[2], `{$1}`)
		registered[match[1]+" "+path] = struct{}{}
	}
	if regexp.MustCompile(`nat\.GET\("/:id/route-suggestions"`).Match(routesSource) {
		registered["GET /network/firewall/nat/{id}/route-suggestions"] = struct{}{}
	}
	...
	if !regexp.MustCompile(`routes\.Use\(middleware\.RequireLocalAdminForWrites\(authService\)\)`).Match(routesSource) {
		t.Error("static route writes are missing local-admin authorization")
	}
}
