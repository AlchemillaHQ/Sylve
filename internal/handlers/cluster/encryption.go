...
func DeleteEncryptionKeyInternal(cS *cluster.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		if cS.Raft != nil && cS.Raft.State() != raft.Leader {
			forwardToLeader(c, cS)
			return
		...
		var req deleteEncryptionKeyRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			c.JSON(http.StatusBadRequest, internal.APIResponse[any]{
				Status:  "error",
				Message: "invalid_request",
		...
		if err := cS.ProposeEncryptionKeyDelete(uuid, cS.Raft == nil); err != nil {
			c.JSON(http.StatusBadRequest, internal.APIResponse[any]{
				Status:  "error",
				Message: "delete_encryption_key_failed",
				Error:   err.Error(),
				Data:    nil,
			})
			return
		...
		c.JSON(http.StatusOK, internal.APIResponse[any]{
			Status:  "success",
			Message: "encryption_key_deleted",
			Data:    nil,
		})
	}
}
