...
export async function loadBackupJobsPageData(vmRid: number = 0) {
	const jobsCacheKey = vmRid > 0 ? `backup-jobs-vm-${vmRid}` : 'backup-jobs';
	const [targets, jobs, nodes, details, basicInfo] = await Promise.all([
		cachedFetch('backup-targets', async () => listBackupTargets(), 1000),
		cachedFetch(jobsCacheKey, async () => listBackupJobs(undefined, vmRid), 1000),
	...
	const effectiveNodes = standaloneMode
