...
export function getJailCreateErrorMessage(
	response: Pick<APIResponse, 'message' | 'error'>
): string {
	const code = getJailCreateErrorCode(response);
	return (
		jailCreateErrorMessageByCode[code] || 'Failed to create jail. Check backend logs for details.'
	);
}
...
export async function removeStaleJailCacheByCTID(ctId: number, hostname: string): Promise<void> {
	const keys = [
		`jail-${ctId}`,
		`simple-jail-${ctId}`,
		`jail-${ctId}-state`,
		...
		'jail-list',
		`simple-jail-list-${hostname}`,
		`jail-list-${hostname}`,
		'simple-jails'
	];

	await Promise.allSettled(keys.map((key) => removeCache(key, hostname)));

	// Clear entries written before caches were scoped by hostname as well.
