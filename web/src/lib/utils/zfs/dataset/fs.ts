...
export function generateTableData(grouped: GroupedByPool[]): { rows: Row[]; columns: Column[] } {
    const columns: Column[] = [
        { field: 'id', title: 'ID', visible: false },
        {
            ...
            formatter: (cell) => {
                ...
                if (encryption && encryption !== 'off') {
                    return renderWithIcon(['material-symbols:lock', 'material-symbols:files'], value);
                }
        ...
        { field: 'encryption', title: 'encryption', visible: false }
    ];
    ...
    for (const group of grouped) {
        ...
        for (const fs of group.filesystems) {
            ...
            node.type = fs.type;
            node.encryption = fs.properties?.encryption || 'off';
