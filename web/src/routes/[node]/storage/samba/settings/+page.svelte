				...
				formatter: (cell: CellComponent) => {
					...
					if (property === 'Interfaces') {
						...
						const formattedValue = arr.map((v: string) => {
							const option = interfaceOptions.find((candidate) => candidate.value === v.trim());
...
<Config bind:open={modalOpen} bind:reload sambaConfig={sambaConfig.current} {interfaceOptions} />
