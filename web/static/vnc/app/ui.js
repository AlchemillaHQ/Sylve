...
const UI = {
	...
	async start(options = {}) {
		...
		if (!window.isSecureContext) {
			...
			UI.showStatus(
				_('Running without HTTPS is not recommended, crashes or other issues are likely.'),
				'error'
			);
		}
	...
	addConnectionControlHandlers() {
		...
		document
			.getElementById('noVNC_approve_server_button')
			.addEventListener('click', UI.approveServer);
		document
			.getElementById('noVNC_reject_server_button')
	...
	idleControlbar() {
		...
		if (
			document.getElementById('noVNC_control_bar').contains(document.activeElement) &&
			document.hasFocus()
		) {
