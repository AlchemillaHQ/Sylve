...
export default class RFB extends EventTargetMixin {
	constructor(target, urlOrChannel, options) {
		...
		this._rfbDisconnectReason = null;
	...
	_socketClose(e) {
		...
		if (e.code) {
			...
			// Check for specific error conditions that indicate VNC port conflicts
			const reasonLower = e.reason ? e.reason.toLowerCase() : '';

			if (
				// Check for our specific 409 error message
				reasonLower.includes('vnc port is already in use') ||
				reasonLower.includes('already in use by another client') ||
				// Check for various ways the 409 error might be conveyed
				e.code === 4409 ||
				reasonLower.includes('409') ||
				reasonLower.includes('conflict') ||
				// Some servers might send these close codes for resource conflicts
				e.code === 1013 || // Try again later
				(e.code === 1006 && reasonLower.includes('conflict'))
			) {
				userFriendlyMsg =
					'VNC session is already in use by another client. Please wait and try again.';
			}
		}

		const errorMessage = userFriendlyMsg || 'Connection closed ' + msg;

		switch (this._rfbConnectionState) {
			case 'connecting':
				this._fail(errorMessage);
				break;
			case 'connected':
	...
	_socketError(e) {
		...
		if (e && e.target && e.target.readyState === WebSocket.CLOSED) {
			// Connection failed during handshake
			// The actual error details might be in the close event that follows
			Log.Debug('WebSocket error during connection attempt');
		}
	}
				...
				this.dispatchEvent(
					new CustomEvent('disconnect', {
						detail: {
							...
							reason: this._rfbDisconnectReason
						}
					})
				);
				break;
	...
	_fail(details) {
		...
		this._rfbDisconnectReason = details;
